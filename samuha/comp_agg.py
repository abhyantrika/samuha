...
class CA(object):
	...
	n_clusters: No of clusters required. (int)
				This algorithm selects the optimal number of clusters for your dataset.
	...
	Use case:
			...
			g = CA(data,n_clusters=4,m=1.5,eps=1e-3,gamma=0.1,beta=1e10,eta=6,tau=10,cardinality_min=1e3)
			fuzzy_matrix ,cost_function,cluster_assignments = g.predict()

	"""
	def __init__(self,data,n_clusters=2,m=2.0,eps=1e-4,gamma=0,beta=1e15,eta=5,tau=10,cardinality_min = 1e3,dims=None):
	...
	def predict(self,max_iterations=None):
		...
		#Initial large value of alpha
		self.alpha = (np.sum(f0,0) * np.sum(d,0)) / np.sum(f0,0)
		self.alpha = np.sum(self.alpha) * 10000
		...
		while self.get_criterion(max_iterations,iteration,f0,f):
			...
			#Expo decay
			self.eta = self.eta * (np.exp(-iteration/self.tau)/10)
			self.alpha = self.eta * self.alpha
			...
			distout = np.sqrt(d)
			cost_function.append(np.sum(np.sum(f0*d,0),0) - self.alpha*np.sum(np.sum(f0,0)))
			...
			#Cardinality
			deleted_clusters = []
			cardinality = np.sum(f0,0)
			for card in range(len(cardinality)):
				if cardinality[card] < self.cardinality_min:
					deleted_clusters.append(card)

			#Delete clusters below thresh
			f0 = np.delete(f0,deleted_clusters,1)
			f = np.delete(f,deleted_clusters,1)
			d = np.delete(d,deleted_clusters,1)
			self.n_clusters = self.n_clusters - len(deleted_clusters)
			print(deleted_clusters)

			try:
				cluster_assignments = np.apply_along_axis(np.argmax,1,f0)
			except:
				cluster_assignments = np.zeros(len(self.data))
